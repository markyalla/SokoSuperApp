...
func main() {
	...
	worker.StartPickupTimeoutWatcher(dbs.Shopper, dbs.Delivery)
	...
	v1 := r.Group("/api/v1")
	{
		...
		webhooksGroup := v1.Group("/webhooks")
		{
			...
			webhooksGroup.POST("/paystack", webhooks.HandlePaystack(dbs))
		...
		deliveryGroup.Use(middleware.JWTAuthMiddleware())
		{
			...
			driver.Use(middleware.AuthorizeRoles(models.RoleDriver, models.RoleSuperAdmin))
			{
				...
				driver.PATCH("/deliveries/:id/status", delivery.UpdateStatus(dbs.Delivery))
				...
				driver.PATCH("/orders/:id/tracking", shopper.UpdateOrderTracking(dbs.Shopper))
